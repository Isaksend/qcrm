...
tenge-denominated revenue environments, or maintain rudimentary
spreadsheet pipelines that preclude any form of predictive analysis.

Machine learning has become a central component of modern CRM systems, 
enabling predictive modelling of customer behaviour such as conversion 
likelihood and churn risk. In this project, predictive models are integrated 
directly into the CRM workflow to support sales prioritisation and retention 
strategies. Predictive lead scoring enables sales teams to concentrate effort 
on the highest-value opportunities, materially improving funnel conversion 
\citep{verbeke2012}. Customer churn analysis supports proactive retention 
interventions that the literature demonstrates can reduce churn rates 
by 20--30\% \citep{verbeke2012}. These capabilities are increasingly 
realisable through mature open-source frameworks such as scikit-learn 
\citep{chen2016xgboost} and interpretability toolkits such as SHAP \citep{lundberg2017shap}.
...
\begin{enumerate}
  ...
  locally dominant communication channels or Kazakhstani payment gateway
  integration.

  \item \textbf{Infrastructure complexity.} Open-source self-hosted
  alternatives such as SuiteCRM require significant infrastructure
...
regulations, and deployable without infrastructure management overhead?}
...
churn analysis. The platform targets Kazakhstani SMEs and is deployed on
Serverspace.io (vStack, Almaty, Kazakhstan), a domestic VPS provider whose
Almaty data centre satisfies Law~No.~94-V data residency requirements
\citep{serverspacedocs2025,kazakhstanlaw94v}.

\section{Objectives}

\begin{enumerate}[label=\textbf{O\arabic*.}]
  \item Conduct a systematic literature review of CRM architectures, SaaS
  design patterns, lead scoring methodologies, and churn prediction algorithms.
  \item Analyse five representative existing CRM systems to identify feature
  gaps relevant to the Kazakhstani SME context.
  ...
  \item Produce complete UML documentation expressed as PlantUML source:
  Use Case, Class, Sequence, Activity, Component, and Deployment diagrams.
  \item Deploy the MVP on Serverspace.io and evaluate system performance under
...
\begin{itemize}
  ...
  \item \textbf{Database:} A PostgreSQL~16 relational database 
  \item \textbf{Deployment:} Serverspace.io web service backed by a
  Docker Compose containers deployment. 
  \item \textbf{Testing:} Unit tests using \texttt{pytest}, API integration
  tests via Postman/Newman, and load testing via Locust.
\end{itemize}

\subsection{Limitations}

\begin{itemize}
  \item ML models are trained on publicly available datasets (the IBM Telco
  Customer Churn dataset \citep{ibmtelco} and a HubSpot developer sandbox
  lead dataset \citep{hubspot_leads}) in the absence of real Kazakhstani
  CRM data; production performance on local data may differ.
  \item The monolithic architecture is appropriate for MVP scale but will
  require migration to a modular or microservices pattern for production
  scalability; this migration is explicitly out of scope.
  \item Kazakh-language UI localisation is partially implemented; NLP
  features for Kazakh-language activity notes are deferred to future work.
  \item Email and telephony integrations are mocked within the MVP.
  \item Serverspace.io Almaty data centre is located within Kazakhstan, satisfying data residency requirements; future
  within Kazakhstan would require migration to a local provider to satisfy
  Law~No.~94-V data residency requirements in a production context.
  \item Security hardening beyond HTTPS enforcement, bcrypt password
  hashing, and JWT expiry has not been subjected to independent
  penetration testing.
\end{itemize}
